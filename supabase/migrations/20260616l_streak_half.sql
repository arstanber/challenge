-- Streak threshold: a day counts at "at least half" of the day's recurring
-- tasks (was 75% rounded down). Threshold = ceil(0.5 * scheduled), min 1:
--   1->1  2->1  3->2  4->2  5->3  6->3  8->4
-- Completing half (or more) of the day now keeps the streak. Redefines the two
-- functions that encode the threshold (latest in 20260616g_streak_75_floor.sql).
-- The Swift mirror (Constants.dailyStreakGoal / TaskEngine fallback) is updated
-- to 0.5 + ceil in the same change -- keep them in sync.
...
begin
  ...
  return v_done >= greatest(1, ceil(0.5 * v_scheduled))::int;
end $$;
...
begin
  ...
  qual as (
    ...
    where coalesce(dn.cnt, 0) >= greatest(1, ceil(0.5 * s.scheduled))::int
    union
