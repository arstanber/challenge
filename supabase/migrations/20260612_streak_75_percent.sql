-- Global streak rule change: a day qualifies when the user completed at least
-- 75% of the recurring tasks scheduled for that day (minimum 1), instead of a
-- flat ">= 3 distinct activities".
--
-- Denominator (scheduled count) for a day D, in the user's timezone:
--   active recurring activities (frequency <> 'once') created on or before D
--   whose schedule_days is NULL/empty or contains D's ISO weekday.
-- One-off tasks still count toward the numerator (they are real work done)
-- but never inflate the denominator. Days with no recurring roster fall back
-- to "did at least 1 task".
--
-- The p_min parameter is kept for signature compatibility with
-- refresh_my_streaks / get_leaderboard (both pass 3) but is no longer used.
-- Qualifying-report rule is unchanged: approved/not_applicable/pending count,
-- rejected never counts, today without a report does not break the run yet.
