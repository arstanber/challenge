...
- No `print()` — use `os.Logger`: `import os.log`, then `private let logger = Logger(subsystem: "com.reinspire", category: "<ClassName>")`. Use `logger.error(...)` for errors, `logger.debug(...)` for debug info. File-level `private let` for enums/static types, stored property for classes/structs.
