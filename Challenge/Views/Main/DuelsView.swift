...
private struct DuelInviteSheet: View {
    ...
    private var shareText: String {
        "Вызываю тебя на дуэль в The Challenge ⚔️ Каждый день закрываем по задаче -- кто сорвётся, тот проиграл. Код: \(code). Скачай: https://thechallenges.app"
    }
...
struct DuelDetailView: View {
    ...
    private var pendingSection: some View {
        VStack(spacing: 14) {
            ...
            ShareLink(item: "Вызываю тебя на дуэль в The Challenge ⚔️ Код: \(current.inviteCode). Скачай: https://thechallenges.app") {
                Label("Отправить вызов", systemImage: "paperplane.fill")
                    .frame(maxWidth: .infinity)
                    .frame(height: 46)
            }
